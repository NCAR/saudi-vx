...
#                will be formatted: b - beginning, e - end
#      lead/accum:
...
sub vx_run_cmd {
  ...
  # store begin time
  my $beg = strftime("%s", gmtime());

  # run the command
  print "CALLING: $command\n";
  my $status = system ( $command );

  print "Command ran for ", strftime("%s", gmtime()) - $beg, " seconds.\n";

  # check for bad return status
  if ( $status != 0 ) {
    print "WARNING: Command returned with non-zero ($status) status...\n",
        "  $command\n";
  }
